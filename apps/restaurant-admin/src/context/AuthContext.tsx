...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Also handles ?impersonationToken= from the super-admin "impersonate" flow.
  useEffect(() => {
    let cancelled = false;

    // Check for an impersonation token in the URL first (takes priority).
    ...
    if (impToken) {
      ...
      try {
        ...
        if (claims.imp && claims.slug) {
          ...
          setStoredTenantSlug(claims.slug);
        ...
        // Malformed token — fall through to normal flow.
      }
    }
